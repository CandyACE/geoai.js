...
<Callout type="info">
  **Core Components:** Pipeline execution engine, extensible provider system,
  and modular model architecture supporting both Transformers.js and ONNX
  models.
</Callout>
...
<Callout type="info">
  **Key Extension:** `GeoRawImage` extends `RawImage` with geospatial metadata
  including bounds, transform, and CRS information.
</Callout>
...
<Callout type="info">
  **Supported Providers:** Geobase (custom COG imagery), Mapbox (global
  satellite), with Google Maps and Esri ArcGIS coming soon.
</Callout>
...
<Callout type="info">
  **Two Patterns:** Single task execution for individual AI models, and task
  chaining for complex analysis workflows where output from one model becomes
  input to the next.
</Callout>
...
<Callout type="info">
  **Required:** Geographic polygon defining the analysis area. **Optional:**
  Post-processing and map source parameters for fine-tuning model behavior.
</Callout>
